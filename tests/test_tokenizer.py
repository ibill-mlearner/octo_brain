import sys
import unittest
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from tentacles.tokenizer import SpatialTokenizer


class SpatialTokenizerTest(unittest.TestCase):
    def test_raw_sensor_values_are_normalized_without_semantic_tokens(self):
        tokenizer = SpatialTokenizer(add_eos=True)

        values = tokenizer.normalize_raw_values([-10, 0, 127.5, 255, 300])

        # This is what I expect to happen, values should equal [0.0, 0.0, 0.5, 1.0, 1.0].
        self.assertEqual(values, [0.0, 0.0, 0.5, 1.0, 1.0])

    def test_raw_values_are_placed_into_spatial_frames(self):
        tokenizer = SpatialTokenizer(window_size=(2, 2, 1), add_eos=False)

        frames = tokenizer.raw_values_to_frames([0, 127.5, 255, 64, 32], origins=[(0, 0, 0), (10, 0, 0)])

        # This is what I expect to happen, len(frames) should equal 2.
        self.assertEqual(len(frames), 2)
        # This is what I expect to happen, frames[0].values should equal (0.0, 0.5, 1.0, 64 / 255).
        self.assertEqual(frames[0].values, (0.0, 0.5, 1.0, 64 / 255))
        # This is what I expect to happen, frames[0].coordinates should equal ((0, 0, 0), (1, 0, 0), (0, 1, 0), (1, 1, 0)).
        self.assertEqual(frames[0].coordinates, ((0, 0, 0), (1, 0, 0), (0, 1, 0), (1, 1, 0)))
        # This is what I expect to happen, frames[1].values should equal (32 / 255,).
        self.assertEqual(frames[1].values, (32 / 255,))
        # This is what I expect to happen, frames[1].coordinates should equal ((10, 0, 0),).
        self.assertEqual(frames[1].coordinates, ((10, 0, 0),))

    def test_coordinates_fill_x_then_y_then_z(self):
        tokenizer = SpatialTokenizer(window_size=(10, 10, 10), add_eos=False)

        coords = tokenizer.coordinates_for_count((20, 30, 40), 12)

        # This is what I expect to happen, coords[0] should equal (20, 30, 40).
        self.assertEqual(coords[0], (20, 30, 40))
        # This is what I expect to happen, coords[9] should equal (29, 30, 40).
        self.assertEqual(coords[9], (29, 30, 40))
        # This is what I expect to happen, coords[10] should equal (20, 31, 40).
        self.assertEqual(coords[10], (20, 31, 40))
        # This is what I expect to happen, coords[11] should equal (21, 31, 40).
        self.assertEqual(coords[11], (21, 31, 40))

    def test_raw_values_to_frames_requires_enough_origins(self):
        tokenizer = SpatialTokenizer(window_size=(2, 2, 1), add_eos=False)

        # This is what I expect to happen, the next operation should raise ValueError.
        with self.assertRaises(ValueError):
            tokenizer.raw_values_to_frames([0, 1, 2, 3, 4], origins=[(0, 0, 0)])

    def test_raw_value_frame_processing_maps_one_coordinate_per_reading(self):
        # This is what I expect to happen, this small test-only tokenizer should
        # behave exactly like SpatialTokenizer while keeping a count of coordinate
        # lookups so the test can speculate about processing cost without relying
        # on fragile timing measurements.
        class CountingTokenizer(SpatialTokenizer):
            def __init__(self, *args, **kwargs):
                super().__init__(*args, **kwargs)
                # This is what I expect to happen, the counter should start at
                # zero before raw sensor readings are projected into frames.
                self.local_coordinate_calls = 0

            def local_coordinate(self, local_index):
                # This is what I expect to happen, each raw reading should ask
                # for exactly one local coordinate, so this counter should grow
                # at the same rate as the input list rather than multiplying by
                # image-like channels, kernels, or nested processing layers.
                self.local_coordinate_calls += 1
                return super().local_coordinate(local_index)

        raw_values = list(range(10))
        tokenizer = CountingTokenizer(window_size=(4, 1, 1), add_eos=False)

        frames = tokenizer.raw_values_to_frames(
            raw_values,
            origins=[(0, 0, 0), (10, 0, 0), (20, 0, 0)],
        )

        # This is what I expect to happen, sum(len(frame.values) for frame in frames) should equal len(raw_values).
        self.assertEqual(sum(len(frame.values) for frame in frames), len(raw_values))
        # This is what I expect to happen, sum(len(frame.coordinates) for frame in frames) should equal len(raw_values).
        self.assertEqual(sum(len(frame.coordinates) for frame in frames), len(raw_values))
        # This is what I expect to happen, tokenizer.local_coordinate_calls should equal len(raw_values).
        self.assertEqual(tokenizer.local_coordinate_calls, len(raw_values))

    def test_text_round_trip_remains_for_debugging(self):
        tokenizer = SpatialTokenizer(add_eos=True)
        text = "scanner memory: hello π"

        token_ids = tokenizer.encode(text)

        # This is what I expect to happen, token_ids[-1] should equal SpatialTokenizer.EOS.
        self.assertEqual(token_ids[-1], SpatialTokenizer.EOS)
        # This is what I expect to happen, tokenizer.decode(token_ids) should equal text.
        self.assertEqual(tokenizer.decode(token_ids), text)


class DesktopSensorProbeTest(unittest.TestCase):
    def test_probe_readings_can_be_turned_into_raw_values(self):
        from sensors.interfaces import SensorReading, readings_to_spatial_values

        readings = [SensorReading("cpu", 12.5, "%"), SensorReading("memory", 2048.0, "MB")]

        # This is what I expect to happen, readings_to_spatial_values(readings) should equal [12.5, 2048.0].
        self.assertEqual(readings_to_spatial_values(readings), [12.5, 2048.0])


if __name__ == "__main__":
    unittest.main()
